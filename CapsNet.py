...
def augmentation(x, max_shift=2):
    ...
    #当随机shift为正（+n）：0 -> len-n 替代 n -> len
    #当随机shift为负（-n）：n -> len 替代 0 -> len-n
...
class CapsuleNet(nn.Module):
    def __init__(self):
        ...
        # self.conv1 = nn.Conv2d(in_channels=2, out_channels=256, kernel_size=3, stride=1)
        ...
        # self.primary_capsules = CapsuleLayer(num_capsules=8*2, num_route_nodes=-1, in_channels=256*2, out_channels=32*2,
        #                                      kernel_size=3, stride=2)
        # self.digit_capsules = CapsuleLayer(num_capsules=NUM_CLASSES, num_route_nodes=32 * 7 * 7 *2, in_channels=8*2,
        #                                    out_channels=16*2)
        self.primary_capsules = CapsuleLayer(num_capsules=16, num_route_nodes=-1, in_channels=64, out_channels=4,
                                             kernel_size=9, stride=2)
        self.digit_capsules = CapsuleLayer(num_capsules=NUM_CLASSES, num_route_nodes=4 * 4 * 4, in_channels=16,
                                           out_channels=32)
...
if __name__ == "__main__":
    ...
    def get_iterator(mode):
        ...
        #
        # train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=mode)
        # test_loader = DataLoader(test_dataset, batch_size=BATCH_SIZE, shuffle=mode)
        #
        if mode:
            data = train_data
            labels = train_label
